...
def digital_glitch_effect(device, original_image, glitch_intensity=3):
    """
    Efecto de glitch digital como el del GIF
    glitch_intensity: 1 (suave) a 5 (fuerte)
    """
    import random
    width, height = original_image.size
    
    while True:
        # Mostrar imagen normal la mayor parte del tiempo
        device.image(original_image)
        device.show()
        time.sleep(random.uniform(0.5, 0.5))  # Tiempo aleatorio entre glitches
        
        # Aplicar glitch aleatorio
        if random.random() > 0.7:  # 30% de probabilidad de glitch
            # Crear múltiples bandas horizontales desplazadas
            glitch_image = original_image.copy()
            
            # Número de bandas a desplazar (más intensidad = más bandas)
            num_bands = random.randint(1, glitch_intensity * 3)
            
            for _ in range(num_bands):
                # Seleccionar una banda horizontal aleatoria
                band_height = random.randint(1, 10)
                y_pos = random.randint(0, height - band_height)
                
                # Desplazamiento horizontal aleatorio
                shift = random.randint(-glitch_intensity * 5, glitch_intensity * 5)
                
                # Cortar y desplazar la banda
                band = original_image.crop((0, y_pos, width, y_pos + band_height))
                shifted_band = band.transform((width, band_height), Image.AFFINE, 
                                            (1, 0, shift, 0, 1, 0))
                
                # Pegar la banda desplazada
                glitch_image.paste(shifted_band, (0, y_pos))
            
            # Mostrar el glitch brevemente
            device.image(glitch_image)
            device.show()
            time.sleep(random.uniform(0.05, 0.2))  # Breve duración del glitch
...
print("Imagen de Batman mostrada")
time.sleep(2)  # Espera 2 segundos para ver la imagen estática

print("Activando efecto glitch digital...")
digital_glitch_effect(oled, imagen_final, glitch_intensity=3)

print("Servicio OLED activo. Esperando...")
